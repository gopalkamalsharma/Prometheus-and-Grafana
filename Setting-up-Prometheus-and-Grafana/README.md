...
![Prometheus Architecture](architecture.gif)
...
```bash
eksctl create cluster --name=monitoring \
                      --region=ap-south-1 \
...
```
```bash
eksctl create nodegroup --cluster=monitoring \
                        --region=ap-south-1 \
                        --name=monitoring-ng-private \
                        --node-type=t2.medium \
                        --nodes-min=2 \
                        --nodes-max=3 \
                        --node-volume-size=20 \
...
aws eks update-kubeconfig --name monitoring
```
...
```
```bash
# Download the kube-prometheus-stack.yml file
...
```bash
eksctl delete cluster --name monitoring
```
